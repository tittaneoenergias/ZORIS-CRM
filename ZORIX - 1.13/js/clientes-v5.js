...
class ClientesManagerV5 {
    ...
    async handleFileUpload(event, clienteId) {
        ...
        // Para cada arquivo, salvar tanto na lista do cliente quanto no sistema geral
        Array.from(files).forEach(async (file) => {
            ...
            try {
                if (window.api) {
                    ...
                    // Fallback: salvar diretamente no localStorage do sistema
                    const arquivosSistema = JSON.parse(localStorage.getItem('lurix_arquivos') || '[]');
                    arquivosSistema.push(arquivoSistema);
                    localStorage.setItem('lurix_arquivos', JSON.stringify(arquivosSistema));
                }
