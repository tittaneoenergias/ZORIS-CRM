...
class ConfiguracoesManager {
                        ...
                        <div class="bg-gradient-to-r from-orange-500 to-red-600 text-white p-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-orange-100 mt-2">Gerencie cadastros pendentes de aprovação</p>
                                </div>
                                <div class="flex items-center space-x-4">
        ...
        if (pendingUsers.length === 0) {
            return `
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500">Todos os usuários foram aprovados ou não há cadastros pendentes</p>
                </div>
            `;
        }
    ...
    refreshPendingUsers() {
        ...
        if (container) {
            ...
            this.showNotification('Lista de usuários atualizada!', 'info');
        }
    }
    ...
    async approveUser(userId, userName) {
        if (!confirm(`Deseja aprovar o cadastro de "${userName}"?\n\nO usuário poderá fazer login no sistema após a aprovação.`)) {
            return;
        }

        try {
            // Usar a função de aprovação do sistema de autenticação
            if (window.auth && typeof window.auth.approvePendingUser === 'function') {
                ...
                this.showNotification(`Usuário "${userName}" aprovado com sucesso!`, 'success');
            ...
            this.showNotification(`Erro ao aprovar usuário: ${error.message}`, 'error');
        }
    }
    ...
    async rejectUser(userId, userName) {
        const reason = prompt(`Motivo da rejeição para "${userName}":\n(Opcional)`);
        ...
        try {
            // Usar a função de rejeição do sistema de autenticação
            if (window.auth && typeof window.auth.rejectPendingUser === 'function') {
                ...
                // Enviar email de rejeição (simulado)
            ...
            this.showNotification(`Erro ao rejeitar usuário: ${error.message}`, 'error');
        }
    }
    ...
    /**
     * Simula envio de email de aprovação
     */
    sendApprovalEmail(userName) {
        console.log(`📧 Email de aprovação enviado para: ${userName}`);
    ...
    /**
     * Simula envio de email de rejeição
     */
    sendRejectionEmail(userName, reason) {
        console.log(`📧 Email de rejeição enviado para: ${userName}. Motivo: ${reason || 'Não especificado'}`);
