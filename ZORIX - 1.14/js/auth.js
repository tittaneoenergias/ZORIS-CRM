...
class Auth {
    ...
    async register(nome, email, tipo, senha) {
        try {
            ...
            // Configurar contexto do usuário no storage para nova empresa
            if (window.storage && window.storage.setUserContext) {
                window.storage.setUserContext(newUser.id);
                // Limpar dados para começar do zero para nova empresa
                window.storage.clearUserData();
                window.storage.initializeUserData();
                console.log('🏢 Nova empresa criada com sistema zerado');
            }
            if (window.zorixStorage && window.zorixStorage.setUserContext) {
                window.zorixStorage.setUserContext(newUser.id);
                console.log('🏢 ZORIX Storage configurado para nova empresa');
            }
